...
export default function QrCode_Text() {
  ...
  return (
    <div className='flex items-center justify-center w-full h-full'>
      <div className='relative left-[930px] top-[80px] h-[650px] w-[250px] pr-[30px] bg-[#191919] border border-black'>
        ...
        <Link to="/qrcode-TEXT" className='font-poppins drop-shadow-[10px] text-shadow-white text-white antialiased'>SMS</Link>
      ...
      <div className='grid grid-cols-1'>
        ...
        <div className='flex justify-center items-center w-[650px] h-[100px] mb-[20px] mt-[40px] relative border border-black bg-[#939393]'>
          ...
          <div>
            <h1 className='absolute left-4 text-white'>TEXT:</h1>
            <input type='text' onChange={(element) => {
                setText(element.target.value)
            } } className='absolute bottom-[10px] left-[120px] w-[500px] h-[70px] bg-white opacity-100'/>
          </div>
        </div>

        <div className='flex justify-center items-center border border-black w-[650px] h-[650px] bg-white'>
          <QRCode level='H' size={qrSize} value={text} bgColor={bgColor} fgColor={fgColor} imageSettings={{
            src: qrImg,
